...
pub mod raw {
    ...
    use crate::vm::types::{
        ASCIIData, BuffData, BufferLength, CharType, FixedFunction, FunctionArg, FunctionSignature,
        FunctionType, ListData, ListTypeData, OptionalData, ResponseData, SequenceData,
        SequenceSubtype, StandardPrincipalData, StringSubtype, StringUTF8Length, TraitIdentifier,
        TupleData, TupleTypeSignature, TypeSignature, UTF8Data,
    };
    use crate::vm::{ClarityName, ContractName, SymbolicExpression, SymbolicExpressionType, Value};
    ...
    fn random_type_signature<R: Rng + ?Sized>(level: Option<u32>, rng: &mut R) -> TypeSignature {
        ...
        let mut signatures = vec![
            TypeSignature::SequenceType(SequenceSubtype::StringType(StringSubtype::ASCII(
                BufferLength(rng.gen_range(5..50)),
            ))),
            TypeSignature::SequenceType(SequenceSubtype::StringType(StringSubtype::UTF8(
                StringUTF8Length(rng.gen_range(5..50)),
            ))),
            ...
            if lvl < MAX_RECURSION_LEVEL {
                let mut recursive_vals = vec![
                    ...
                    Value::Optional(OptionalData {
                        data: if level.is_none() {
                            Some(Box::new(random_value(next_level, rng)))
                        } else {
                            None
                        },
                    }),
